...
static int default_mode = DEVICE_MODE_ISDBT/*DEVICE_MODE_NONE*/;
...
int smscore_register_device(struct smsdevice_params_t *params,
	...
	if (!dev->common_buffer) {
		sms_info("dma_alloc_coherent(...) failed");
	...
	for (buffer = dev->common_buffer; dev->num_buffers <
		...
		if (!cb) {
			sms_info("smscore_createbuffer(...) failed");
			smscore_unregister_device(dev);
			return -ENOMEM;
		}
...
//	sms_info("before init_completion ~!!");

	init_completion(completion);

//	sms_info("after init_completion ~!!");
...
int smscore_start_device(struct smscore_device_t *coredev)
{
	...
	if (rc < 0) {
		...
		printk("set device mode to %d failed", mode);
		printk("board_id=%d, default_mode=%d",board_id,sms_get_board(board_id)->default_mode);
		/* 
...
//jason110921	 mode = sms_get_board(board_id)->default_mode;
	...
	printk("set device mode succeeded");
	
	rc = smscore_configure_board(coredev);
	if (rc < 0) {
		sms_info("configure board failed , rc %d", rc);
		printk("configure board failed , rc %d", rc);
		return rc;
	}
#endif
	...
	printk("device 0x%p started, rc %d", coredev, rc);
...
static int smscore_load_firmware_family2(struct smscore_device_t *coredev,
	...
	if (!msg)
	{
		sms_info("kmalloc fail");//(msg=%d)",msg);
		return -ENOMEM;
	}
	sms_info("coredev->mode=%d",coredev->mode);
	
	if (coredev->mode != DEVICE_MODE_NONE) {
		sms_info("sending reload command");
	...
	while (size && rc >= 0) {
		...
		//sms_info("1. payload_size=%d, size=%d, rc=%d",payload_size,size,rc);
		...
		//sms_info("2. payload_size=%d, size=%d, rc=%d",payload_size,size,rc);
//		msleep(0);		
	}
...
//	const struct firmware *fw;
...
#if 0//jason110210 for FW with header file
	...
	fw_buf_size = fw->size;
#endif	//jason110210 for FW with header file
...
//jason110210 for FW with header file	kfree(fw_buf);

#ifdef REQUEST_FIRMWARE_SUPPORTED
//jason110210 for FW with header file	release_firmware(fw);
#else
	coredev->fw_buf = NULL;
...
//			mdelay(100);
//			rc = smscore_detect_mode(coredev);//jason110214 for test must removed
		}
...
static int __init smscore_module_init(void)
{
	...
	printk("[SMS2130] smschar registered\n");
	rc = smschar_register();
